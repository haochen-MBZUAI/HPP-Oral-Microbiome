...
def load_gene_family_data(input_dir):
    ...
    for arrow_file in tqdm(arrow_files, desc="Loading Arrow files"):
        ...
        try:
            ...
            # First two columns are ID and collection_date
            if df.shape[1] < 3:
                continue
    ...
    # All Arrow files should have the same ID and collection_date column names
    if not all_data:
        return None
    
    combined_df = all_data[0]
    id_cols = combined_df.columns[:2].tolist()  # participant_id and collection_date
