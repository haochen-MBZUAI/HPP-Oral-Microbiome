...
# ==============================================================================
INPUT_FILE = "home/ec2-user/Studies/Oral_HPP/oral_data/regression_result/strain_phenotype_regression_results_corrected.csv"
DOMAIN_MAP_FILE = '../oral_features_classfication/feature.csv'
OUTPUT_FILE = './key_strain_features.csv'
...
def load_and_prepare_data(input_file, phenotype_to_system):
    ...
    if 'Strain_Feature' not in df.columns:
        print(f"Error: 'Strain_Feature' column not found")
        return None
...
def calculate_association_breadth(df):
    ...
    breadth_df = df.groupby(['Strain_Feature', 'system'])['Phenotype_Feature'].nunique().reset_index()
    breadth_df.columns = ['Strain_Feature', 'system', 'association_count']
    return breadth_df
...
def select_top_features(ranked_df, top_n=5):
    ...
    for system in ranked_df['system'].unique():
        ...
        top_system_features = system_df.head(top_n)['Strain_Feature'].tolist()
...
# 4. MAIN EXECUTION
# ==============================================================================
def identify_key_strain_features():
    """Main function to identify key strain features."""
    print("=" * 80)
    print("Identifying Key Strain Features")
    print("=" * 80)
    ...
    print("\nStep 2: Loading and preparing strain data...")
    strain_df = load_and_prepare_data(INPUT_FILE, phenotype_to_system)
    
    if strain_df is None or strain_df.empty:
        print("  No strain data available. Exiting.")
        return
    
    print(f"  Loaded {len(strain_df)} significant strain associations")
    
    print("\nStep 3: Calculating association breadth...")
    strain_breadth = calculate_association_breadth(strain_df)
    print(f"  Found {len(strain_breadth)} feature-system combinations")
    
    print("\nStep 4: Ranking features by association count...")
    strain_ranked = rank_features_by_breadth(strain_breadth)
    
    print("\nStep 5: Selecting top features per system...")
    strain_top = select_top_features(strain_ranked, top_n=TOP_N_FEATURES_PER_SYSTEM)
    
    key_features = []
    for feature in strain_top:
        feature_info = strain_ranked[strain_ranked['Strain_Feature'] == feature].iloc[0]
        key_features.append({
            'Strain_Feature': feature,
            'System': feature_info['system'],
            'Association_Count': feature_info['association_count']
        })
    
    print(f"  Selected {len(strain_top)} top strain features")
    
    print("\nStep 6: De-duplicating features across systems...")
    key_features_df = pd.DataFrame(key_features)
    
    deduplicated = key_features_df.groupby('Strain_Feature').agg({
    ...
    print(f"\n--- Summary ---")
    print(f"Total key strain features: {len(deduplicated)}")
