...
# 3. 读取并整合数据 (修正后的变换逻辑)
# -------------------------------------------------------------------
pathway_data <- read.csv("gene_family_Bonferroni.csv")
...
# The heatmap still needs the original proportion, so we join it back in
# We also need to handle the NA cases from the gaps we added
heatmap_data <- data_long %>%
  distinct(id, .keep_all = TRUE) %>%
  select(id, phenotype) %>%
  left_join(select(data, phenotype, positive_prop), by = "phenotype")
...
grid_levels_orig <- c(30000, 60000, 100000)
...
# 定义动态的内圈元素尺寸
heatmap_height <- max_val_trans * 0.047
...
p_final <- ggplot(data_long) +
  ...
  # Label Layers
  geom_text(data = label_data, aes(x = id, y = total_value + 50, label = Suggested.Name, hjust = hjust), color = "black", fontface = "bold", alpha = 0.9, size = 3.8, angle = label_data$angle, inherit.aes = FALSE) +
...
# 8. 保存并显示图形
# -------------------------------------------------------------------
ggsave(p_final, file = "./正式结果/gene_family.png", width = 14, height = 14, dpi = 600, bg = "white")
print(p_final)