...
def merge_with_time_window(df_microbiome, df_phenotype, time_window_days=180):
    ...
    Args:
        df_microbiome: DataFrame with microbiome data (must have 'participant_id' and 'collection_date' columns)
        df_phenotype: DataFrame with phenotype data (must have 'participant_id' and 'collection_date' columns)
    ...
    if 'collection_date' in df_microbiome.columns:
        df_microbiome = df_microbiome.copy()
        df_microbiome['collection_date'] = pd.to_datetime(df_microbiome['collection_date'], errors='coerce')
    
    if 'collection_date' in df_phenotype.columns:
        df_phenotype = df_phenotype.copy()
        df_phenotype['collection_date'] = pd.to_datetime(df_phenotype['collection_date'], errors='coerce')
    ...
    if 'collection_date_microbiome' in merged.columns and 'collection_date_phenotype' in merged.columns:
        merged['date_diff'] = (merged['collection_date_microbiome'] - merged['collection_date_phenotype']).dt.days.abs()
        ...
        # Keep only one collection_date column (use microbiome's)
        if 'collection_date_microbiome' in merged.columns:
            merged['collection_date'] = merged['collection_date_microbiome']
            merged = merged.drop(columns=['collection_date_microbiome', 'collection_date_phenotype', 'date_diff'])
    elif 'collection_date' in merged.columns:
    ...
    # But keep collection_date if it was renamed
    cols_to_drop = [col for col in cols_to_drop if col != 'collection_date_microbiome' and col != 'collection_date_phenotype']
    ...
    overlapping_cols.discard('collection_date')
