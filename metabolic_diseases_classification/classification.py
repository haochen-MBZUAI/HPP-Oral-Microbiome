...
# --- Feature layer: 'pathway' or 'strain' (must match select_diseases_associated_oral_features.py LAYER) ---
...
COL_SIGNIFICANT_FEATURE = 'Significant_Pathway' if LAYER == 'pathway' else 'Significant_Strain'
...
# Canonical fold split (run generate_fold_splits.py first so same train/test as select_diseases_associated_oral_features.py)
...
# If set to True, oral marker lists from _fold{n}.csv will be prioritized within each fold
...
# --- STEP 8: 5-FOLD CROSS-VALIDATION (use canonical fold split if available) ---
print(f"\n--- STEP 8: 5-Fold Cross-Validation (MODEL: {MODEL_TO_USE}) ---")

if os.path.exists(FOLD_SPLITS_FILE):
    fold_splits_df = pd.read_csv(FOLD_SPLITS_FILE)
    use_canonical_splits = True
    print(f"Using canonical fold splits from {FOLD_SPLITS_FILE} (aligned with select_diseases_associated_oral_features.py).")
else:
    use_canonical_splits = False
    skf = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE)
    splits_list = list(skf.split(X, y))
    print("Fold splits file not found; using StratifiedKFold (fold indices will NOT align with select script).")

acc_list, auc_list = [], []

for fold_idx in range(1, N_SPLITS + 1):
    if use_canonical_splits:
        train_ids = set(fold_splits_df[(fold_splits_df['fold'] == fold_idx) & (fold_splits_df['role'] == 'train')]['participant_id'])
        test_ids = set(fold_splits_df[(fold_splits_df['fold'] == fold_idx) & (fold_splits_df['role'] == 'test')]['participant_id'])
        train_mask = df_model_data['participant_id'].isin(train_ids)
        test_mask = df_model_data['participant_id'].isin(test_ids)
        train_idx = df_model_data.index[train_mask].tolist()
        test_idx = df_model_data.index[test_mask].tolist()
    else:
        train_idx, test_idx = splits_list[fold_idx - 1]
        train_idx, test_idx = train_idx.tolist(), test_idx.tolist()

    print(f"\n===== Fold {fold_idx}/{N_SPLITS} =====")
    X_train = X.loc[train_idx].copy() if use_canonical_splits else X.iloc[train_idx].copy()
    X_test = X.loc[test_idx].copy() if use_canonical_splits else X.iloc[test_idx].copy()
    y_train = y.loc[train_idx] if use_canonical_splits else y.iloc[train_idx]
    y_test = y.loc[test_idx] if use_canonical_splits else y.iloc[test_idx]

    # --- [Per Fold] Load feature selection list on demand (Priority: Per-fold file -> Summary file -> All features) ---
    if USE_FEATURE_SELECTION:
        # Generate per-fold filename
        if IMPORTANT_FEATURES_FILE.endswith('.csv'):
            per_fold_path = IMPORTANT_FEATURES_FILE.replace('.csv', f'_fold{fold_idx}.csv')
        else:
            per_fold_path = IMPORTANT_FEATURES_FILE + f'_fold{fold_idx}.csv'
        ...
        try:
            if os.path.exists(per_fold_path):
                ...
                selected_cols = [c for c in df_imp[COL_SIGNIFICANT_FEATURE].tolist() if c in X_train.columns]
                print(f"Applied per-fold {LAYER} list: {per_fold_path} -> {len(selected_cols)} features")
            elif os.path.exists(IMPORTANT_FEATURES_FILE):
                df_imp = pd.read_csv(IMPORTANT_FEATURES_FILE)
                selected_cols = [c for c in df_imp[COL_SIGNIFICANT_FEATURE].tolist() if c in X_train.columns]
                print(f"Per-fold list not found. Fallback to union file: {IMPORTANT_FEATURES_FILE} -> {len(selected_cols)} features")
            else:
                print(f"WARNING: {LAYER} list not found. Using all features.")
        except Exception as e:
            print(f"WARNING: Failed to read {LAYER} file ({e}). Using all features.")
            selected_cols = None
        ...
        if selected_cols and len(selected_cols) > 0:
            ...
            print(f"NOTE: No matching {LAYER} columns found in feature matrix. Using all features.")
