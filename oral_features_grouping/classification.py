...
# ==============================================================================
# !! IMPORTANT !!
# Use Bonferroni-corrected regression results (output of correct_P_value_strain_pathway.py).
# For pathway: pathway_phenotype_regression_results_corrected.csv
# For strain: strain_phenotype_regression_results_corrected.csv
# INPUT_ASSOCIATION_FILE_PATH = "home/ec2-user/Studies/Oral_HPP/oral_data/regression_result/strain_phenotype_regression_results_corrected.csv"
INPUT_ASSOCIATION_FILE_PATH = "home/ec2-user/Studies/Oral_HPP/oral_data/regression_result/pathway_phenotype_regression_results_corrected.csv"
DOMAIN_MAP_FILE = './feature.csv'

# -- Output Files --
OUTPUT_PREFIX = './pathway'
FAVOURABLE_OUTPUT_FILE = f'{OUTPUT_PREFIX}_Favourable.csv'
ADVERSE_OUTPUT_FILE = f'{OUTPUT_PREFIX}_Adverse.csv'
MIXED_OUTPUT_FILE = f'{OUTPUT_PREFIX}_Mixed.csv'

# -- Classification Thresholds --
FAVOURABLE_THRESHOLD = 0.90
...
def classify_pathways(df, favourable_thresh, adverse_thresh, print_scores=False):
    ...
    for pathway in unique_pathways:
        ...
        if all(s >= favourable_thresh for s in scores):
            pathway_classifications[pathway] = 'Favourable'
        elif all(s <= adverse_thresh for s in scores):
            pathway_classifications[pathway] = 'Adverse'
        else:
            pathway_classifications[pathway] = 'Mixed'
...
if __name__ == '__main__':
    ...
    if master_df is not None and not master_df.empty:
        ...
        favourable_list = sorted([p for p, c in final_classifications.items() if c == 'Favourable'])
        adverse_list = sorted([p for p, c in final_classifications.items() if c == 'Adverse'])
        mixed_list = sorted([p for p, c in final_classifications.items() if c == 'Mixed'])

        print("\n--- Classification Summary ---")
        print(f"Favourable Pathways: {len(favourable_list)}")
        print(f"Adverse Pathways: {len(adverse_list)}")
        print(f"Mixed Pathways: {len(mixed_list)}")
        print("--------------------------\n")

        pd.DataFrame({'Pathway_Feature': favourable_list}).to_csv(FAVOURABLE_OUTPUT_FILE, index=False)
        print(f"Saved Favourable pathways to '{FAVOURABLE_OUTPUT_FILE}'")

        pd.DataFrame({'Pathway_Feature': adverse_list}).to_csv(ADVERSE_OUTPUT_FILE, index=False)
        print(f"Saved Adverse pathways to '{ADVERSE_OUTPUT_FILE}'")
